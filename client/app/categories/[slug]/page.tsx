...
async function fetchProductsByCategory(slug: string) {
  const res = await fetch(
    `${API_BASE}/products/?category=${encodeURIComponent(slug)}&page_size=48&status=ACTIVE`,
    { next: { revalidate: 600 } }
  );
  if (!res.ok) return [];
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = `${category.name} - Mua ${category.name} chính hãng`;
  const description =
    category.description ||
    `Mua ${category.name} chính hãng tại BanThuoc. Đa dạng sản phẩm, giá tốt, giao hàng nhanh toàn quốc.`;
...
export default async function CategoryDetailPage({ params }: Props) {
  ...
  const collectionLd = {
    ...
    name: category.name,
    description:
      category.description || `Danh mục ${category.name} tại BanThuoc`,

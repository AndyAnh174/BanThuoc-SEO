                    ...
                    <div className="relative py-2">
                         <DropdownMenu onOpenChange={setIsCategoryMenuOpen}>
                            ...
                            <DropdownMenuContent align="start" className="w-[92vw] max-w-[1300px] p-0 rounded-2xl shadow-2xl border-gray-100 flex items-start min-h-[380px] max-h-[80vh] overflow-hidden mt-3 animate-in fade-in-0 zoom-in-95" sideOffset={8}>
                             ...
                             <div className="w-[320px] border-r border-gray-100 bg-gray-50/80 backdrop-blur-sm py-3 shrink-0 h-full flex flex-col gap-1 overflow-y-auto">
                                ...
                                >
                                    ...
                                    {cat.children && cat.children.length > 0 && (
                                        <ChevronDown className={`w-3 h-3 -rotate-90 transition-all ${activeCategory?.slug === cat.slug ? 'text-primary translate-x-1' : 'text-gray-300'}`} />
                                    )}
                                </Link>
                                ))}

                                <div className="mt-auto px-3 pt-3 pb-1.5">
                                     <Link href="/categories" className="flex items-center justify-center gap-1.5 p-2.5 text-xs font-semibold text-primary bg-primary/5 hover:bg-primary/10 rounded-xl border border-primary/10 transition-colors w-full">
                                         Xem tất cả danh mục
                             ...
                             <div className="flex-1 bg-white p-5 h-full flex flex-col overflow-y-auto">
                                {activeCategory ? (

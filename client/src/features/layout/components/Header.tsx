...
export function Header({ cartItemCount: initialCount = 0 }: HeaderProps) {
  ...
  const maxVisibleCategories = isLargeScreen ? 10 : 6;
                ...
                <div className="flex items-center gap-1">
                    ...
                    <div className="relative py-2">
                         <DropdownMenu onOpenChange={setIsCategoryMenuOpen}>
                            ...
                            <DropdownMenuContent align="start" className="w-[90vw] max-w-[1200px] p-0 rounded-2xl shadow-2xl border-gray-100 flex items-start min-h-[350px] max-h-[80vh] overflow-hidden mt-3 animate-in fade-in-0 zoom-in-95" sideOffset={8}>
                             {/* Left Side: Category List */}
                             <div className="w-[300px] border-r border-gray-100 bg-gray-50/80 backdrop-blur-sm py-3 shrink-0 h-full flex flex-col gap-1 overflow-y-auto">
                                <Link 
                                    href="/products" 
                                ...
                                {categories.slice(0, maxVisibleCategories).map((cat) => (
                                <Link 
                                    key={cat.slug}
                             ...
                             <div className="flex-1 bg-white p-8 h-full flex flex-col overflow-y-auto">
                                {activeCategory ? (
